...
## Rodando a feature "Gift List Suggestions" localmente

Especificação completa em [specs/001-gift-list-suggestions/](specs/001-gift-list-suggestions/). Stack: Next.js (pages router) + PostgreSQL, sem ORM — ver [docs/CONVENTIONS.md](docs/CONVENTIONS.md).

1. `npm install`
2. `npm run dev` — sobe o Postgres local (Docker Compose), roda as migrations, sincroniza o usuário sysadmin com `ADMIN_EMAIL`/`ADMIN_PASSWORD` de `.env.development` e inicia `next dev` em `http://localhost:3000`. Mudou essas variáveis? Na próxima subida o e-mail/senha do admin no banco são atualizados para bater (ver [infra/scripts/seed-admin.js](infra/scripts/seed-admin.js)).
3. Para o fluxo de compartilhamento (User Story 2), configure chaves de teste do Stripe em `.env.development`:
   - `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` — de uma conta Stripe em **modo de teste**.
   - `STRIPE_HOSTING_FEE_PRICE_ID` — um Price de assinatura recorrente criado no modo de teste.
   - Para testar o webhook localmente: `stripe listen --forward-to localhost:3000/api/v1/billing/webhook`.
4. `npm test` roda a suíte de testes (integration-first, banco real) — ver [quickstart.md](specs/001-gift-list-suggestions/quickstart.md) para os cenários de validação manual ponta a ponta.
